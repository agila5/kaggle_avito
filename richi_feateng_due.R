...
#' trasformazione param_n in factor e poi ricodificati su integer

combi = combi %>%
  mutate(param_1 = fct_lump(as.factor(param_1), n = 200) %>% as.character(),
         param_2 = fct_lump(as.factor(param_2), n = 250) %>% as.character(),
         param_3 = fct_lump(as.factor(param_3), n = 1200) %>% as.character())  %>%
  replace_na(list(param_1 = "Missing",
                  param_2 = "Missing",
                  param_3 = "Missing")) %>%
  mutate(param_1 = as.factor(param_1),
         param_2 = as.factor(param_2),
         param_3 = as.factor(param_3))
...
# salvo
save(yTrain , XTrain, file = "sparse_train_due")
save(XTest, file = "sparse_test_due")
